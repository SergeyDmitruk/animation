...
/* additionnal params */
shakingRepetition = 0;//def (3) amount of time we want the shaking animation to repeat ( doesn't affect the duration of the whole animation, it'll just shake more frequently )
...
timelineShacking
    .fromTo("#bike", 0.02, { x: 10, y: 10 }, { x: 5, y: -5, repeat: 1, repeatDelay: 0.1, yoyo: true, rotation: -1 }, 0)
...
class Slideshow {
    constructor(el) {
        this.items = Array.from(el.querySelectorAll('#bike'));
        this.initEvents();
        this.current = 0;
        startAnimation(`bike-${this.current + 1}`);
    }
    initEvents() {
        this.items.forEach((item, pos) => () => this.navigate(pos));
    }
};

new Slideshow(document.querySelector('body'));
